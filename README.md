...
`demo/` is the project that export is built from. It expects the addon at `res://addons/radi_ot/`, which is
...
```powershell
robocopy . demo\addons\radi_ot /MIR /XD "$PWD\.git" "$PWD\.github" "$PWD\demo" "$PWD\.godot" /XF .gitignore .gitattributes
```
